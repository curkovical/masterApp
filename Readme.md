...
Run setup.sh with `32` or `64` (os architecture) as a parameter.
After a while installation of `Sencha Cmd` will start. Script installs necessary programs:
- docker
- jre-11
- sencha cmd

To download `Ext JS` framework go to https://www.sencha.com/products/extjs/communityedition.

## Run
Navigate to osrm folder and start osrm local server:
```
docker run -t -i -p 5000:5000 -v $(pwd):/data osrm/osrm-backend:latest osrm-routed /data/louisiana-latest.osrm
```
In repo folder, run application by typing:
```
sencha app watch
```
Application should be launched at:
```
http://localhost:1841
```
#
...
This area is intended for overrides like these:
    ...
    Ext.define('masterApp.overrides.foo.Bar', {
